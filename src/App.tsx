...
import { createEntity, createRow, deleteRow, getTreeRows, updateRow } from './api';
...
const App = () => {
  ...
  const handleChangeRow = useCallback(async (value: Partial<TreeRows>) => {
    if (!entityId) return;
    if (value.id) {
      ...
      if (result) {
        setTreeRows((tree) => !tree?.id ? result : addRowToTree(tree, result))
      }
    }
  }, [entityId]);
  ...
  return (
    <>
      ...
      <div className={styles.layout}>
        <Sidebar />
        <Main>
          {treeRows && <Row onDelete={handleDelete} itemOnEditId={itemOnEdit} handleSetItemOnEditId={setItemOnEdit} level={1} onSubmit={handleChangeRow} value={treeRows} initiallyOnEdit={!treeRows?.id} />}
        </Main>
      </div>
    </>
  );
