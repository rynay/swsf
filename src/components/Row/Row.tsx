...
export const Row: FC<Props> = ({
    ...
    const handleSubmit = useCallback(async (value: Partial<TreeRows>) => {
        setLoading(true)
    ...
    return (
        <>
            <div style={{ paddingLeft: `${level * 2}rem` }} className={styles.buttons} onDoubleClick={itemOnEditId ? undefined : handleEdit}>
                <div className={classNames(styles.buttons__container, { [styles.buttons__container_inactive]: onEdit || itemOnEditId })} onClick={(e) => e.stopPropagation()}>
                    {level !== 1 && (
                        <div className={styles.branch} />
                    )}
                    <button className={styles.buttons__add} onClick={() => setOnAddNew(true)}>
                        <ListIcon />
                    </button>
            ...
            {!onEdit && fieldNames.map(field => (
                <div onDoubleClick={itemOnEditId ? undefined : handleEdit} key={field}>
                    {
                        ((loading && (value.id ? itemOnEditId === value.id : itemOnEditId === 'new'))
                        || deleteLoading || initialDeleteLoading)
                        ? (<Placeholder className={styles.placeholder} danger={deleteLoading || initialDeleteLoading} />)
                        : (value[field] ?? <Placeholder className={styles.placeholder} />)
                    }
                </div>
            ))}
            {value.child?.map(el => <Row initialDeleteLoading={deleteLoading || initialDeleteLoading} onDelete={onDelete} itemOnEditId={itemOnEditId} handleSetItemOnEditId={handleSetItemOnEditId} level={level + 1} onSubmit={onSubmit} key={el.id} value={el} />)}
            {onAddNew && <Row onDelete={onDelete} itemOnEditId={itemOnEditId} handleSetItemOnEditId={handleSetItemOnEditId} level={level + 1} onSubmit={handleSubmit} value={{ parentId: value.id }} initiallyOnEdit={true} />}
        </>
    )
}
